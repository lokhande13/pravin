...
createFileWithTableNamesForBuildDB() {
  ...
  mysql -u jboss --password=$(grep mysql.jboss /etc/sysconfig/JunosSpace/pwd | awk -F= '{print $2}') build_db -e "show tables;" > $cacheFile
  #mysql -u jboss --password=$(grep mysql.jboss /etc/sysconfig/JunosSpace/pwd | awk -F= '{print $2}') sm_db -e "show tables;" >> $cacheFile
...
createFileWithTableNamesForSmDB() {
  ...
  mysql -u jboss --password=$(grep mysql.jboss /etc/sysconfig/JunosSpace/pwd | awk -F= '{print $2}') sm_db -e "show tables;" > $cacheFile1
...
queryFunctionForBuildDB() {
	...
	do
                ...
                mysql -u jboss --password=$(grep mysql.jboss /etc/sysconfig/JunosSpace/pwd | awk -F= '{print $2}') build_db -e "select * from $line;" > $allRecordsBuildDB
...
queryFunctionForSmDB() {
        ...
        do
                # do not append to $allRecordsBuildDB. Re-writing to optimize the search time by not increasing the size of $allRecordsBuildDB
                mysql -u jboss --password=$(grep mysql.jboss /etc/sysconfig/JunosSpace/pwd | awk -F= '{print $2}') sm_db -e "select * from $line;" > $allRecordsSmDB
                echo "Reading table => $line => `date`" >> $LOGFILE
                # redirecting it to /dev/null to avoid shitty messages on screen when user runs this script
